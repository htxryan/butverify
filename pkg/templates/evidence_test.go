...
func TestWriteBundleContents_AbortChannelStopsLoopBetweenCopies(t *testing.T) {
	...
	close(abort)
	err := writeBundleContents(in, outDir, dstNames, resolvedSrcs, Generator{}, abort, false, false)
	if err == nil {
...
func TestWriteBundleContents_NilAbortChannelNeverAborts(t *testing.T) {
	...
	if err := writeBundleContents(in, outDir, dstNames, resolvedSrcs, Generator{}, nil, false, false); err != nil {
...
func TestWriteBundleContents_PostLoopAbortCheck(t *testing.T) {
	...
	abort := make(chan struct{})
	close(abort)
	err := writeBundleContents(in, outDir, nil, nil, Generator{}, abort, false, false)
	if !errors.Is(err, errEvidenceAborted) {
