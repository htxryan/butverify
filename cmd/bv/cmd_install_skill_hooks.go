...
// On uncommitted changes it writes a single advisory line to STDERR and
// exits 2. Claude Code injects the hook's STDERR (not stdout) as the
// conversation turn for non-zero exits, so the advisory must go to stderr.
//
