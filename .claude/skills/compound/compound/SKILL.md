...
- Run `go test ./...` and `golangci-lint run ./...` -- must pass
- Read the epic's `## Verification Contract` and run every required evidence item that remains applicable. If `build` is required, run `go build ./...`
