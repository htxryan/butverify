...
- Run targeted test validation using the project's test runner (e.g., `go test ./...` with module/path filters)

### Phase 5: Verification
- Run full test suite (`go test ./...`) and linter (`golangci-lint run ./...`) after changes
...
- **Module isolation**: Run tests for a specific module/path (e.g., `go test ./...` with path filter)
- **Subset testing**: Run a subset of tests for fast feedback (e.g., critical/smoke tests only)
- **Full suite**: Run `go test ./...` for complete validation
