...
- **After Compound (FINAL GATE)**: Run `ca verify-gates <epic-id>` (must PASS), `go test ./...`, and `golangci-lint run ./...`. Then read the epic's `## Verification Contract` and run every required evidence item that remains open, including `go build ./...` when `build` is required, before running `ca phase-check gate final` (auto-cleans phase state)
