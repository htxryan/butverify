...
4. Run baseline quality gates: `go test ./...` and `golangci-lint run ./...`
   - If `Required evidence` includes `build`, also run `go build ./...` before spawning reviewers
...
## Quality Criteria
- Baseline quality gates pass (`go test ./...`, `golangci-lint run ./...`)
- `go build ./...` passed when the Verification Contract required build evidence
