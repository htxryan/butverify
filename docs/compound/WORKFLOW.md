...
- Run baseline quality gates: `go test ./...` and `golangci-lint run ./...`
- If the Verification Contract requires build evidence, also run `go build ./...`
...
| Final | After Compound | `ca verify-gates <epic-id>` passes, `go test ./...` and `golangci-lint run ./...` pass, and the remaining required evidence from the Verification Contract is satisfied (including `go build ./...` when required) |
